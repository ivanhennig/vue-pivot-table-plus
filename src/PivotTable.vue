    ...
    <table v-else class="table table-hover table-bordered table-sm">
      ...
      <thead>
        <tr v-for="(colField, colFieldIndex) in colFields" :key="'header-col-' + colField.label" v-if="colField.showHeader === void 0 || colField.showHeader" class="bg-light">
          <!-- Top left dead zone -->
          <th v-if="colFieldIndex === firstColFieldHeaderIndex && rowHeaderSize > 0" :colspan="rowHeaderSize" :rowspan="colHeaderSize" class="text-right font-weight-normal text-muted">{{ getNumOfCells() }} {{ cellsText }}</th>
          <!-- Column headers -->
